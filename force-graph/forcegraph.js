...
function createNode(parent, { ox, oy, cx, cy, r, id, group, path, queue }, lastTime) {
  let color, textColor;
...
function updateClubNodes(nodes, clubNodes, gc) {
  ...
  for(let i = 0; i < clubNodes.length; i++) {
    for(let j = 0; j < clubNodes.length; j++) {
      ...
      const f = gc * clubNodes[i].count * clubNodes[j].count / dis;

      clubNodes[i] = Object.assign({}, clubNodes[i], {
        cx: clubNodes[i].cx + f * (x / dis),
        cy: clubNodes[i].cy + f * (y / dis),
      });
    }
  }
...
function checkClub(nodes, links) {
  ...
  for (let i = 0; i < l; i++) {
    for (let j = 0; j < l; j++) {
      ...
      const ans = ((linksMap[`${i}-${j}`] === 1 || linksMap[`${j}-${i}`] === 1 ? 1 : 0) - (degreeCount[i] * degreeCount[j] / (2 * m)))
...
function solve(svg, data, testG = 0.81, testGC = 4, record = []) {
